...
`,
      labConfig: {
        ...
        testCases: [{ input: "", expected: "Hello, StudyAI!\n" }],
      },
    },
  });
  ...
  const pyLesson2 = await prisma.lesson.create({
    data: {
      moduleId: pyModule1.id,
...
Use loops to automate repetitive systems tasks.
...
`,
  ...
  const pyLesson3 = await prisma.lesson.create({
    data: {
      moduleId: pyModule1.id,
