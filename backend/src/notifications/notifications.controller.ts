...
class PushSubscriptionDto {
  @ApiProperty({ description: 'Push subscription endpoint URL', example: 'https://fcm.googleapis.com/fcm/send/...' })
  endpoint: string;
...
class SendNotificationDto {
  ...
  @ApiProperty({ description: 'Notification title', example: 'New achievement unlocked!' })
  title: string;

  @ApiProperty({ description: 'Notification body text', example: 'You earned the Streak Master badge.' })
  body: string;

  @ApiProperty({ description: 'Optional notification tag for deduplication', required: false })
  tag?: string;
}
...
class BroadcastDto {
  ...
  @ApiProperty({ description: 'Notification body text', example: 'Scheduled downtime tonight.' })
  body: string;
...
export class NotificationsController {
  ...
  subscribe(
    ...
    this.notificationsService.subscribe(userId, body);
    return { success: true };
  }
  ...
  unsubscribe(
    ...
    const userId = req.user.id;
    this.notificationsService.unsubscribe(userId, body.endpoint);
    return { success: true };
  }
  ...
  getSubscriptions(@Req() req: any) {
    const userId = req.user.id;
    const subs = this.notificationsService.getSubscriptions(userId);
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Send notification to a specific user (self or admin)' })
  @ApiResponse({ status: 200, description: 'Notification sent or access denied.' })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  sendNotification(
    @Req() req: any,
    ...
    if (req.user.id !== body.userId && req.user.role !== 'ADMIN') {
      return { success: false, message: 'Can only send notifications to yourself unless admin' };
    }
  ...
  broadcast(
    ...
    if (req.user.role !== 'ADMIN') {
      return { success: false, message: 'Only admins can broadcast notifications' };
    }
