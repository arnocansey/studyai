...
export class GamificationController {
  ...
  @ApiResponse({
    status: 200,
    description: "Check-in recorded. Returns streak, XP, and new achievements.",
  })
  @ApiResponse({ status: 401, description: "Unauthorized." })
  async checkIn(@Request() req: any) {
    const streakResult = await this.streakService.updateStreak(req.user.id);

    if (streakResult.checkedInToday) {
      const xpProgress = await this.xpService.getXPProgress(req.user.id);
      return {
        streak: streakResult,
        xp: xpProgress,
        xpAwarded: 0,
