...
export function useChatSocket({ userId, userName, serverUrl = 'http://localhost:3001' }: UseChatSocketOptions) {
  ...
  const getHistory = useCallback(
    (room: string, limit?: number): Promise<ChatMessage[]> => {
      return new Promise((resolve) => {
        ...
        socketRef.current.emit('message:history', { room, limit }, (response: { messages: ChatMessage[] }) => {
  ...
  const getRooms = useCallback((): Promise<ChatRoom[]> => {
    return new Promise((resolve) => {
      ...
      socketRef.current.emit('room:list', undefined, (response: { rooms: ChatRoom[] }) => {
