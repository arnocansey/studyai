    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-semibold text-gray-900 dark:text-white">Socratic Tutor</h3>
            <p className="text-xs text-gray-500">I'll guide you to the answer</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && (
          <div className="text-center text-gray-500 py-8">
            ...
            <p className="text-sm mt-1">I'll help you discover the answer rather than just giving it to you.</p>
          </div>
        )}
        ...
        {response?.hint && !loading && (
          ...
          >
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-2xl p-3">
              ...
              <p className="text-sm text-yellow-800 dark:text-yellow-300 mt-1">{response.hint}</p>
            </div>
          </motion.div>
        )}
      </div>
      ...
      <div className="p-4 border-t border-gray-200 dark:border-gray-700">
        ...
        {response?.followUp && !loading && (
          <p className="text-sm text-gray-500 mt-2 italic">💬 {response.followUp}</p>
        )}
      </div>
    </div>
  );
