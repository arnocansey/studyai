...
import { 
  Trophy, Flame, Target, Brain, Code, BookOpen, 
...
export default function GamificationDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'quiz' | 'tutor'>('overview');

  useEffect(() => {
    // Get user from auth context or API
        ...
        {activeTab === 'overview' && (
          ...
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: 'Total XP', value: user?.xp || 0, icon: Zap, color: 'text-purple-500' },
                ...
                { label: 'Courses', value: '3', icon: BookOpen, color: 'text-blue-500' },
              ].map((stat, i) => (
                <motion.div
                  key={stat.label}
                  initial={{ opacity: 0, y: 20 }}
