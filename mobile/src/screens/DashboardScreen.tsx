...
import {
  View, Text, ScrollView, StyleSheet, RefreshControl, TouchableOpacity, useWindowDimensions,
} from 'react-native';
...
export function DashboardScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (xp.status === 'fulfilled') {
        updateUser({ xp: xp.value.totalXp, level: xp.value.level, streak: xp.value.streak });
        await cache.set('dashboard_xp', xp.value);
      } else {
        const cached = await cache.get<any>('dashboard_xp');
        if (cached) {
          updateUser({ xp: cached.totalXp, level: cached.level, streak: cached.streak });
        }
      }
  ...
  const STAT_ICONS = [
    ...
    { emoji: '📚', label: 'Courses completed', value: user.coursesCompleted || 0 },
        ...
        <FadeInView delay={100}>
          <View style={[styles.xpCard, { backgroundColor: colors.accent }]}>
            ...
            <View style={styles.progressBar}>
              <View style={[styles.progressFill, { width: `${Math.max(xpProgress * 100, 2)}%` }]} />
            </View>
            <Text style={styles.xpToNext}>{xpToNext.toLocaleString()} XP to Level {(user.level || 1) + 1}</Text>
          </View>
        </FadeInView>
        ...
        <FadeInView delay={200}>
          <View style={styles.statsGrid}>
            {STAT_ICONS.map((stat, i) => (
              <View
                key={i}
                style={[styles.statCard, { backgroundColor: colors.card, borderColor: colors.border, width: statCardWidth }]}
            ...
            {courses.length > 0 ? (
              courses.map((course) => (
                <TouchableOpacity
                  ...
                  accessibilityRole="button"
                >
                  <View style={[styles.courseIcon, { backgroundColor: colors.accent + '20' }]}>
                    <Text style={styles.courseIconText}>{course.icon}</Text>
                  </View>
                  <View style={styles.courseInfo}>
                    <Text style={[styles.courseTitle, { color: colors.text }]}>{course.title}</Text>
                    <Text style={[styles.courseProgress, { color: colors.textMuted }]}>
                      {course.lessonsCount} lessons · {course.difficulty}
                    </Text>
                  </View>
                </TouchableOpacity>
              ))
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                {achievements.map((achievement) => (
                  ...
                  >
                    ...
                    <Text style={[styles.achievementName, { color: colors.text }]}>{achievement.title}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
            ) : (
...
const styles = StyleSheet.create({
  ...
  xpHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 },
  ...
  statCard: { padding: 16, borderRadius: 16, alignItems: 'center', borderWidth: 1 },
  statIcon: { fontSize: 28, marginBottom: 8 },
  statValue: { fontSize: 22, fontWeight: 'bold' },
  statLabel: { fontSize: 12, marginTop: 4 },
  section: { marginTop: 24, paddingHorizontal: 20 },
  sectionTitle: { fontSize: 20, fontWeight: '700', marginBottom: 15 },
  courseCard: { flexDirection: 'row', borderRadius: 16, padding: 16, alignItems: 'center', borderWidth: 1, marginBottom: 10 },
  ...
  achievementCard: {
    width: 120, padding: 16, borderRadius: 16, alignItems: 'center', marginRight: 12, borderWidth: 1,
  },
  achievementIcon: { fontSize: 36, marginBottom: 8 },
  achievementName: { fontSize: 12, fontWeight: '600', textAlign: 'center' },
  emptyState: {
    padding: 20, borderRadius: 16, borderWidth: 1, alignItems: 'center',
  },
  emptyStateText: { fontSize: 14, textAlign: 'center' },
  offlineBanner: {
    marginHorizontal: 20, marginTop: 10, padding: 12, borderRadius: 12, alignItems: 'center',
  },
