...
import {
  View, Text, ScrollView, TouchableOpacity, StyleSheet, Alert, RefreshControl, Share,
} from 'react-native';
        ...
        {loading ? (
          ...
          <>
            <View style={styles.statsRow}>
              {[
                { label: 'Total', value: String(certificates.length), icon: '📜' },
                { label: 'Verified', value: String(certificates.filter(c => c.verified).length), icon: '✅' },
                { label: 'Skills', value: String(new Set(certificates.flatMap(c => c.skills)).size), icon: '🎯' },
              ].map((stat) => (
            ...
            {certificates.length === 0 ? (
              <View style={{ alignItems: 'center', paddingVertical: 60 }}>
                ...
                <Text style={{ fontSize: 18, fontWeight: '700', color: colors.text, marginBottom: 8 }}>No Certificates Yet</Text>
                <Text style={{ fontSize: 14, color: colors.textMuted, textAlign: 'center', paddingHorizontal: 40 }}>
                  Complete courses to earn verified certificates. Your achievements will appear here.
                </Text>
              </View>
            ) : (
            certificates.map((cert, i) => (
              <FadeInView key={cert.id} delay={i * 80}>
                <TouchableOpacity
                  ...
                  onPress={() => setSelected(selected === cert.id ? null : cert.id)}
                >
                  <View style={styles.certHeader}>
                    <View style={styles.certTitleRow}>
                      <Text style={styles.certEmoji}>{cert.verified ? '🏅' : '📋'}</Text>
                      <View style={{ flex: 1 }}>
                        <Text style={[styles.certTitle, { color: colors.text }]}>{cert.title}</Text>
                        <Text style={[styles.certIssuer, { color: colors.textMuted }]}>{cert.issuer}</Text>
                      </View>
                      {cert.verified && (
                  ...
                  {selected === cert.id && (
                    <View style={[styles.certDetails, { borderTopColor: colors.border }]}>
                      <View style={styles.detailRow}>
                        <Text style={[styles.detailLabel, { color: colors.textMuted }]}>Date</Text>
                        <Text style={[styles.detailValue, { color: colors.text }]}>{cert.date}</Text>
                      </View>
                      <View style={styles.detailRow}>
                        <Text style={[styles.detailLabel, { color: colors.textMuted }]}>Credential ID</Text>
                        <Text style={[styles.detailValue, { color: colors.accent }]}>{cert.credentialId}</Text>
                      </View>
                      <View style={styles.detailRow}>
                        <Text style={[styles.detailLabel, { color: colors.textMuted }]}>Grade</Text>
                        <Text style={[styles.detailValue, { color: colors.text }]}>{cert.grade}</Text>
                      </View>
                      <View style={styles.skillTags}>
                        {cert.skills.map((skill, si) => (
                          <View key={skill} style={[styles.skillTag, { backgroundColor: SKILL_TAGS[si % SKILL_TAGS.length] + '20' }]}>
                            <Text style={[styles.skillTagText, { color: SKILL_TAGS[si % SKILL_TAGS.length] }]}>{skill}</Text>
                          </View>
                        ))}
                      </View>
                      <View style={styles.certActions}>
                        ...
                        <TouchableOpacity
                          ...
                          onPress={() => Share.share({ message: `Check out my certificate: ${cert.title} (${cert.credentialId})`, url: `https://studyai.app/certificates/${cert.credentialId}` })}
                        >
...
const styles = StyleSheet.create({
  ...
  statLabel: { fontSize: 11, marginTop: 2, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.3 },

  certCard: { marginHorizontal: 16, marginBottom: 12, borderRadius: 16, borderWidth: 1, overflow: 'hidden' },
